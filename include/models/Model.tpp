...
namespace tbaricault::models
{
    ...
    {
        constexpr auto index = tbaricault::templates::find<Name, Fields...>::index;
        if constexpr (index == std::numeric_limits<std::size_t>::max())
    ...
    BaseField& Model<Parent, This, Fields...>::get(std::string_view name)
    {
        ...
        ([&] {
            if (Fields::name == name)
                ptr = &std::get<tbaricault::templates::find<Fields::name, Fields...>::index>(_fields);
        }(), ...);
        if (ptr)
            return (*ptr);
